...
pub fn generate_wallpaper(img: Vec<u8>, foreground: image::Rgb<u8>, background: image::Rgb<u8>, size:(usize,usize), padding:(usize, usize, usize, usize)) -> Vec<u8> {
    ...
    let target = image::ImageBuffer::from_fn(size.0 as u32, size.1 as u32, |x, y| {
        ...
        let s = (pixel[0] as f32 - pixel[1] as f32).abs() + (pixel[1] as f32 - pixel[2] as f32).abs() + (pixel[2] as f32 - pixel[0] as f32).abs();

        // If the pixel is saturated, use the original color, otherwise use the foreground or background color
        let v = v / 255.0;
        let s = s / 255.0;
        Rgb([
            ((foreground[0] as f32 * (1.0 - v) + background[0] as f32 * v) * (1.0 - s) + pixel[0] as f32 * s) as u8,
            ((foreground[1] as f32 * (1.0 - v) + background[1] as f32 * v) * (1.0 - s) + pixel[1] as f32 * s) as u8,
            ((foreground[2] as f32 * (1.0 - v) + background[2] as f32 * v) * (1.0 - s) + pixel[2] as f32 * s) as u8,
        ])
    }); 
...
fn parse_rgb(color: &str) -> Result<image::Rgb<u8>, Error> {
    ...
    let caps = re.captures(color).ok_or_else(||Error::ColorParseError(color.to_string()))?;

    let r = caps.get(1).or(caps.get(4)).ok_or_else(||Error::ColorParseError(color.to_string()))?;
    let g = caps.get(2).or(caps.get(5)).ok_or_else(||Error::ColorParseError(color.to_string()))?;
    let b = caps.get(3).or(caps.get(6)).ok_or_else(||Error::ColorParseError(color.to_string()))?;

    let mut r = u8::from_str_radix(r.as_str(), 16).map_err(|_err| Error::ColorParseError(color.to_string()))?;
    let mut g = u8::from_str_radix(g.as_str(), 16).map_err(|_err| Error::ColorParseError(color.to_string()))?;
    let mut b = u8::from_str_radix(b.as_str(), 16).map_err(|_err| Error::ColorParseError(color.to_string()))?;
...
pub fn generate_wallpaper_hex(img: Vec<u8>, foreground: &str, background: &str, size:(usize,usize), padding:(usize, usize, usize, usize)) -> Result<Vec<u8>, Error> {
